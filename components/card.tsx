...
const getParallaxStyle = (x: number, y: number, z: number, i: number) => {
  ...
  const yt = Math.sin(-x * PARALLAX_SPRING_FACTOR) * p * PARALLAX_TRANSLATION_FACTOR;
...
const Card = (props: CardProps) => {
  ...
  const [springProps, springApi] = useSpring<{ xyzs: number[] }>(() => ({
    xyzs: [0, 0, 0, 1],
    ...
    <div className="root">
      <div className="container">
        ...
        <div className="textWrapper">
          <div className={!props.disabled ? "title" : "title disabled"}>{props.title}</div>
  ...
  return (
    <animated.div
      ...
      style={{
        ...
        transform: springProps.xyzs.to((...args: number[]) => {
          const [x, y, , s] = args;
